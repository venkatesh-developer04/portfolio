...
(function ($) {
  ...
  jQuery(function () {
        ...
        if (myCursor.length) {
          if ($('body')) {
            ...
            (window.onmousemove = function (s) {
              ...
              $('body').on('mouseenter', 'button:not(.not-cursor), a:not(.not-cursor), .cursor-pointer', function () {
            ...
            // text view cursor
            $('h1, h2, h3, h4, h5, h6, .cursor-lg').on('mouseenter', function () {
              $('.mouseCursor').addClass('cursor-big');
            });

            $('h1, h2, h3, h4, h5, h6, .cursor-lg').on('mouseleave', function () {
    ...
    function setActiveMenu(container) {
      const currentPath = location.pathname.replace(/^\/|\/$/g, '') || 'index.html';
      ...
      for (const li of container.querySelectorAll('li')) {
        ...
        if (a.getAttribute('href').replace(/^\/|\/$/g, '') === currentPath) {
          li.classList.add('active');
          li.closest('ul')?.closest('li')?.classList.add('active');
          break;
        }
      }
    }
    ...
    if (header) setActiveMenu(header);
    ...
    $('.close-mobile-menu, .mobile-menu__backdrop, .mobile-menu .navbar__item:not(.navbar__item--has-children) > a, .mobile-menu__social').on('click', function () {
      ...
      $('.mobile-menu .navbar__dropdown-label').removeClass('navbar__item-active');
    ...
    function updateSlideNumbers(swiper) {
      ...
      var realSlides = wrapper.querySelectorAll('.swiper-slide:not(.swiper-slide-duplicate)').length;
    ...
    $('.comment-single').each(function () {
      $(this)
        ...
        .on('click', function () {
          ...
          $('.comment-single .reply-comment').not($currentComment.find('.reply-comment')).slideUp();
          $currentComment.find('.reply-comment').slideToggle();
          $('.comment-single .reply-button button').not(this).removeClass('active');
          $(this).toggleClass('active');
        });
    });
    ...
    function initSplitTextAnimation() {
      ...
      $animatedTexts.each(function () {
        ...
        let folksGradient = $el.hasClass('custom-gradient') ? chroma.scale(['#4E92E3', '#F20101']) : chroma.scale(['#C9F31D', '#F20101']);
    ...
    if ($('.progress-bar-single').length > 0) {
      $('[data-percent]').each(function () {
        $(this).find('.progress-bar-percent').css('width', $(this).attr('data-percent'));
        $(this).find('.percent-value').text($(this).attr('data-percent').replace('%', 'm'));
      });
    ...
    function lightMode() {
      ...
      $('.logo-ch').attr('src', 'assets/images/banner/vk_logo.svg');
      $('.pulse-text').hide();
